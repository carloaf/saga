...
<head>
    ...
    <title>Resumo Mensal - {{ $start_date->format('m/Y') }}</title>
    <style>
        body {
...
<body>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th style="text-align: center">Média Diária</th>
            </tr>
        </thead>
        <tbody>
            <tr>
            ...
            <tr class="highlight">
                ...
                <td style="text-align: center"><strong>{{ number_format($data['total_bookings'] / $data['period_days'], 1) }}</strong></td>
            </tr>
        </tbody>
    </table>
    ...
    <div style="margin: 20px 0; padding: 15px; border: 1px solid #ddd; background-color: #f9f9f9;">
        <ul style="margin: 0; padding-left: 20px;">
            <li><strong>Mês analisado:</strong> {{ $start_date->translatedFormat('F \d\e Y') }}</li>
            <li><strong>Total de dias:</strong> {{ $data['period_days'] }} dias</li>
            ...
            @if($data['daily_stats']->count() > 0)
                @php
                    $maxDay = $data['daily_stats']->sortByDesc('total')->first();
                    $minDay = $data['daily_stats']->sortBy('total')->first();
                    $maxDate = \Carbon\Carbon::parse($maxDay->date);
                    $minDate = \Carbon\Carbon::parse($minDay->date);
                @endphp
                <li><strong>Dia com mais agendamentos:</strong> {{ $maxDate->format('d/m/Y') }} ({{ $maxDate->translatedFormat('l') }}) - {{ $maxDay->total }} agendamentos</li>
                <li><strong>Dia com menos agendamentos:</strong> {{ $minDate->format('d/m/Y') }} ({{ $minDate->translatedFormat('l') }}) - {{ $minDay->total }} agendamentos</li>
            @endif
            
            @if($data['breakfast_count'] > $data['lunch_count'])
                <li><strong>Tendência predominante:</strong> Café da manhã ({{ number_format(($data['breakfast_count'] / $data['total_bookings']) * 100, 1) }}%)</li>
            @elseif($data['lunch_count'] > $data['breakfast_count'])
                <li><strong>Tendência predominante:</strong> Almoço ({{ number_format(($data['lunch_count'] / $data['total_bookings']) * 100, 1) }}%)</li>
            @else
    ...
    <div class="footer">
        ...
        <p>Documento gerado automaticamente - Para uso interno administrativo</p>
    </div>
</body>
</html>
